...
dispatch decision, so it lives in `cpustate.js`; `pcjsvax-bc3` owns the CIS arithmetic itself, which
a VMS-booting machine will need and this diagnostic does not.

Still missing: devices (no Qbus, no console, no interval timer — EHKAA takes no device interrupt,
which is why it runs without them), packed-decimal/CIS arithmetic (`pcjsvax-bc3`), and H_floating
