 ...
 *
 * `fOmitCdg` (pcjsvax-fe7, --no-cdg) leaves the cache-diagnostic space UNDECODED.  It touches
 * exactly one line of this function and nothing anywhere else -- deliberately, because PHASE F (the
 * regression floor) uses it to hand the walk rule a REAL hardware gap and then requires the walk to
 * stop at it and name it.  A floor that reached into the walk rule itself would grade nothing.
 *
...
function makeMachine(romBytes, fOmitCdg = false)
{
    ...
    if (!fOmitCdg) bus.addCdg(new CDGVAX(ka655));
    cpu.setBus(bus);
