...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="APPCLASS">vax</xsl:variable>
	<xsl:variable name="APPNAME">VAXjs</xsl:variable>
	<xsl:variable name="APPVERSION">1.00</xsl:variable>
	...
	<!--
		pcjsvax-f23.  PDPjs and PCx86 emit a BUNDLED release script here
		(/machines/dec/pdp11/releases/2.21/pdp11.js), produced by this repo's gulp build.  VAXjs has
		no bundle yet, so this emits the UNBUNDLED entry point instead, which is not an invention:
		it is the same shape PCjs's own `site.pcjs.unbundled` mode emits from
		_includes/machines/scripts.html (`<script type="module" src="..."/>`).  One module is named
		rather than the ~30 that mode lists, because ES module imports resolve the rest.

		A `type="module"` script is deferred, so it evaluates BEFORE the window 'load' event that
		weblib.js:1202 hooks to WebLib.doPageInit(), which is what makes every component's
		WebLib.onInit() handler fire on schedule.
	-->
	<xsl:template name="componentScripts">
		<xsl:param name="component"/>
		<script type="module" src="/machines/{$MACHINECLASS}/{$APPCLASS}/modules/v2/machine.js"> </script>
	</xsl:template>
	...
	<xsl:template match="cpu[not(@ref)]">
		...
		<!--
			pcjsvax-f23.  `autoBoot` is VAX-specific and is DELIBERATELY separate from `autoStart`:
			autoStart decides whether the CPU runs, autoBoot decides whether the console auto-boot
			script types `B DUA0` and answers OpenVMS's date prompt.  A visitor who wants a bare
			`>>>` still gets a running machine.  See modules/v2/cpuka655.js.
		-->
		<xsl:variable name="autoBoot">
			<xsl:choose>
				<xsl:when test="@autoboot"><xsl:value-of select="@autoboot"/></xsl:when>
				<xsl:when test="@autoBoot"><xsl:value-of select="@autoBoot"/></xsl:when>
